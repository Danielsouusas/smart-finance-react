...
const App = () => {
  ...
  const resumo = useMemo(() => {
    ...
    return { ent: totalEntradasMes, sai: totalSaidasMes, lista: transacoes, entDia, saiDia };
  ...
  const dadosGrafico = useMemo(() => {
    return resumo.lista.map(t => {
      ...
      return { 
        dia: t.data ? t.data.substring(8, 10) : "", 
        entrada: t.tipo === 'entrada' ? v : 0, 
        saida: t.tipo === 'saida' ? v : 0 
      };
    });
  }, [resumo.lista]);
    ...
    return (
      <div style={{ backgroundColor: '#000814', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', fontFamily: 'monospace' }}>
        <div style={{ backgroundColor: '#001d3d', padding: '40px', borderRadius: '20px', width: '90%', maxWidth: '350px', border: '1px solid #007BFF', textAlign: 'center', boxShadow: '0 0 30px #007BFF' }}>
          <h2 style={{ color: '#007BFF', textShadow: '0 0 10px #007BFF' }}>SMART-GDTEC: INTELIGÊNCIA FINANCEIRA</h2>
          <form onSubmit={(e) => { e.preventDefault(); if (usuario === 'daniel' && senhaInput === '4321') setLogado(true); else alert('ACESSO NEGADO'); }}>
            <input type="text" placeholder="USER" onChange={e => setUsuario(e.target.value)} style={{ width: '100%', padding: '12px', marginBottom: '10px', backgroundColor: '#000', color: '#007BFF', border: '1px solid #007BFF' }} />
            <input type="password" placeholder="PASS" onChange={e => setSenhaInput(e.target.value)} style={{ width: '100%', padding: '12px', marginBottom: '20px', backgroundColor: '#000', color: '#007BFF', border: '1px solid #007BFF' }} />
            <button type="submit" style={{ width: '100%', padding: '12px', backgroundColor: '#007BFF', fontWeight: 'bold', border: 'none', color: '#fff', cursor: 'pointer', boxShadow: '0 0 10px #007BFF' }}>ACESSAR</button>
          </form>
        </div>
      </div>
    );
    ...
    <div style={{ backgroundColor: '#0a0a0a', color: '#e0e0e0', minHeight: '100vh', padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: 'auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #333', paddingBottom: '10px', marginBottom: '20px', alignItems: 'center' }}>
          <h2 style={{ color: '#00d1b2', fontSize: '1.2rem', textShadow: '0 0 10px #00d1b2' }}>SMART-GDTEC: INTELIGÊNCIA FINANCEIRA</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#161616', padding: '20px', borderRadius: '10px', border: '1px solid #00d1b2', boxShadow: '0 0 15px rgba(0, 209, 178, 0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <small style={{ color: '#00d1b2', textShadow: '0 0 10px #00d1b2', fontWeight: 'bold' }}>PROGRESSO DIÁRIO</small>
              <small style={{ color: '#00d1b2', textShadow: '0 0 10px #00d1b2', fontWeight: 'bold' }}>{((resumo.entDia / 400) * 100).toFixed(0)}%</small>
            </div>
            <div style={{ width: '100%', backgroundColor: '#333', height: '12px', borderRadius: '6px', overflow: 'hidden', marginBottom: '10px' }}>
              <div style={{ width: `${Math.min((resumo.entDia / 400) * 100, 100)}%`, height: '100%', backgroundColor: '#00d1b2', boxShadow: '0 0 10px #00d1b2' }} />
            </div>
            <h2 style={{ color: '#00d1b2', textShadow: '0 0 12px #00d1b2', margin: '5px 0' }}>
              {resumo.entDia >= 400 ? '✅ META BATIDA!' : `R$ ${Mascarar(resumo.entDia.toFixed(2))}`}
            </h2>
          </div>

          <div style={{ backgroundColor: '#161616', padding: '20px', borderRadius: '10px', border: '1px solid #00d1b2', boxShadow: '0 0 15px rgba(0, 209, 178, 0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <small style={{ color: '#00d1b2', textShadow: '0 0 10px #00d1b2', fontWeight: 'bold' }}>PROGRESSO MENSAL</small>
              <small style={{ color: '#00d1b2', textShadow: '0 0 10px #00d1b2', fontWeight: 'bold' }}>{visivel ? ((resumo.ent / 10000) * 100).toFixed(1) : '**'}%</small>
            </div>
            <div style={{ width: '100%', backgroundColor: '#333', height: '12px', borderRadius: '6px', overflow: 'hidden' }}>
              <div style={{ width: `${Math.min((resumo.ent / 10000) * 100, 100)}%`, height: '100%', backgroundColor: '#00d1b2', boxShadow: '0 0 10px #00d1b2' }} />
            </div>
            <p style={{ color: '#00d1b2', textShadow: '0 0 12px #00d1b2', fontSize: '1.5rem', margin: '10px 0 0 0', fontWeight: 'bold' }}>
              R$ {Mascarar(resumo.ent.toLocaleString('pt-BR'))}
            </p>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: '#161616', padding: '20px', borderRadius: '10px', border: '1px solid #333', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
            <h4 style={{ margin: 0, color: '#00d1b2', textShadow: '0 0 10px #00d1b2' }}>RESUMO DO DIA: {data.split('-').reverse().join('/')}</h4>
          ...
          <div style={{ margin: '15px 0', fontSize: '1.1rem' }}>
            <span>Entradas: <b style={{color: '#00d1b2'}}>R$ {Mascarar(resumo.entDia.toFixed(2))}</b></span>
            <span style={{ margin: '0 15px', color: '#333' }}>|</span>
            <span>Saídas: <b style={{color: '#ff3860'}}>R$ {Mascarar(resumo.saiDia.toFixed(2))}</b></span>
          </div>
          <button onClick={enviarWhatsApp} style={{ backgroundColor: '#25d366', border: 'none', padding: '12px 20px', cursor: 'pointer', borderRadius: '5px', fontWeight: 'bold', color: '#000', width: '100%' }}>
              ENVIAR NO WHATSAPP
          </button>
        </div>
                ...
                {resumo.lista.slice().reverse().map(t => (
                  <tr key={t.id} style={{ borderBottom: '1px solid #222' }}>
                    ...
                    <td style={{ textAlign: 'right', padding: '12px', color: t.tipo === 'entrada' ? '#00d1b2' : '#ff3860', fontWeight: 'bold' }}>
                      {t.tipo === 'entrada' ? '+' : '-'} {Mascarar(Number(t.valor).toFixed(2))}
                    </td>
